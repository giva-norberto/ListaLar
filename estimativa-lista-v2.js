...
//
// Evolução principal:
// - conciliação por família de produto antes da similaridade textual;
// - tabela de aliases para descrições abreviadas de nota fiscal;
// - proteção contra famílias incompatíveis (ex.: peito x asa de frango);
// - mantém quadro de estimativa e selo R$ por item reconhecido.
// ==========================================

import {
  classificarFamiliaProduto,
  familiasSaoIncompativeis
} from "./familias-produtos.js?v=1.0.0";
...
const PALAVRAS_IGNORADAS = new Set([
  ...
  "PCT", "PCTE", "PACOTE", "PACOTES", "PT", "CX", "CAIXA", "CAIXAS",
  "BDJ", "BANDEJA", "BJ", "LATA", "LATAS", "VD", "VIDRO", "FR", "FRASCO",
  "GF", "TB", "TP"
]);
...
function tokenEhMedida(token) {
  const texto = String(token || "");
  return (
    /^\d+(?:KG|G|GR|ML|L|LT|UN|UND|PCT|PT|CX|BJ|GF|TB|TP)$/.test(texto) ||
    /^\d+$/.test(texto)
  );
}
...
function prefixoComum(a, b) {
  ...
  while (quantidade < limite && a[quantidade] === b[quantidade]) quantidade += 1;
  return quantidade;
}
...
function pontuarCorrespondencia(produto, historico) {
  ...
  const familiaLista = classificarFamiliaProduto(produto?.nome);
  const familiaFiscal = historico?.familia || classificarFamiliaProduto(historico?.nome);

  if (!nomeLista || !nomeFiscal) return { pontuacao: 0, criterio: "" };
  if (nomeLista === nomeFiscal) return { pontuacao: 1, criterio: "nome_exato" };

  if (familiaLista && familiaFiscal) {
    if (familiaLista.id === familiaFiscal.id) {
      return {
        pontuacao: 0.97,
        criterio: "familia_produto",
        familia: familiaLista
      };
    }

    if (familiasSaoIncompativeis(familiaLista, familiaFiscal)) {
      return { pontuacao: 0, criterio: "familia_incompativel" };
    }
  }

  if (!tokensLista.length || !tokensFiscal.length) {
    return { pontuacao: 0, criterio: "" };
  }
  ...
  if (tokensLista.length === 1) {
    ...
    return melhor >= 0.74
      ? { pontuacao: melhor, criterio: "similaridade_textual" }
      : { pontuacao: 0, criterio: "" };
  }
  ...
  for (const tokenLista of tokensLista) {
    ...
    if (melhorIndice < 0 || melhorSimilaridade < 0.70) {
      return { pontuacao: 0, criterio: "" };
    }
  ...
  return media >= 0.72
    ? { pontuacao: media, criterio: "similaridade_textual" }
    : { pontuacao: 0, criterio: "" };
}
...
function localizarPrecoDetalhado(produto) {
  ...
  if (produtoId) {
    ...
    if (porId?.preco > 0) {
      return { ...porId, confianca: 1, criterio: "produto_id" };
    }
  }

  const porNome = mapaExato.get(chaveExataNome(produto?.nome));
  if (porNome?.preco > 0) {
    return { ...porNome, confianca: 1, criterio: "nome_exato" };
  }

  let melhor = null;
  let melhorPontuacao = 0;
  let melhorCriterio = "";
  let melhorFamilia = null;

  for (const historico of itensHistoricos) {
    const resultado = pontuarCorrespondencia(produto, historico);
    const pontuacao = numeroSeguro(resultado?.pontuacao, 0);
    ...
    if (
      ...
      melhorCriterio = resultado.criterio || "";
      melhorFamilia = resultado.familia || historico.familia || null;
    }
  }
  ...
  return melhor
    ? {
        ...
        criterio: melhorCriterio || "descricao_fiscal",
        familiaConciliada: melhorFamilia
      }
    : null;
}
...
function renderizarIndicadoresHistorico() {
  ...
  area.querySelectorAll(".buy-item").forEach((linha) => {
    ...
    const familia = registro.familiaConciliada || registro.familia;
    indicador.title =
      `Preço histórico: ${formatarMoeda(registro.preco)}` +
      (familia?.nome ? ` · família: ${familia.nome}` : "") +
...
function renderizarEstimativa() {
  ...
  for (const produto of lista) {
    const registro = localizarPrecoDetalhado(produto);
    const preco = numeroSeguro(registro?.preco, 0);
  ...
  if (itensComPreco === 0) {
    ...
    if (infoEl) {
      infoEl.textContent = "Nenhum item da lista foi conciliado com o histórico ainda.";
    }
  } else {
    ...
    resultados
      ...
      .forEach((gasto) => {
        gasto.itens.forEach((item) => {
          ...
          const familia = classificarFamiliaProduto(nome);
          const registro = {
            preco,
            data: gasto.data,
            nome,
